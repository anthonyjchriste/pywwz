...
c----------------------------------------------------------------------------
c
c     WEIGHTED WAVELET Z-TRANSFORM
c
c     This is a fortran version of Grant Foster's WWZ1.1.BAS BASIC program,
c     stripped down for speed and readability improvements.  It is modestly
c     less flexible than the basic code (esp regarding varying input file
c     formats), but will be easy to modify to suit your needs.
c
c     In the event you have more than 100000 data points, you can resize all
c     arrays to meet your needs -- the only limitation is your free memory.
c     (Note that all variables are double precision, though.)
c
c     A description of the mathematics can be found in G. Foster, "Wavelets
c     for Period Analysis of Unevenly Sampled Time Series", Astronomical Journal
c     112, 1709 (Oct 1996).
c
c		-Matthew Templeton, August 15, 2002
c----------------------------------------------------------------------------

c print copyright statement from original program
...
c end of program
      ...
      subroutine dataread
c reads in a data file
c note: this assumes your data are in two columns: time(1) and mag(2)
c it can be easily changed by buffering columns with dummy variables -- just
c declare "real*8 dum" and read in null columns with that variable.

c this subroutine also returns the number of data points (numdat)
c the average (dave), the variance (dvar), and the standard deviation (dsig),
c along with the name of the input file.
      ...
      do i=1,100000
       ...
       numdat=numdat+1
      enddo
c when eof(1) is reached, the loop jumps here
999   continue
      close(1)
...
c open your output file (there is only one output file in this version)
      ...
      subroutine maketau
c make your array of time lags, tau, here.  In BASIC, you could just step from
c whatever taus you choose, like "FOR DTAU = DTAU1 TO DTAU2 STEP DT", but FORTRAN
c only lets you loop over integers.  So we determine the taus ahead of time.
...
c rounds the taus... from G. Foster's code.
      implicit none
      real*8 dex,nex,darg
      dex=dlog10(darg)
      nex=dint(dex)
      ...
      subroutine makefreq
c query the user for a frequency range, and make an array of frequencies
      implicit none
...
c depending upon the user inputs, you may wind up with a lot of frequencies.
c this will query in case nfreq>1000 (nfreq=1000 is a reasonable number)
...
701    print*,'you have more than 1000 frequencies.  OK? (y/n)'
      ...
      subroutine getcon
c query for a decay constant, C (as in exp(-c*omega^2)), for the wavelet window
      implicit none
      real*8 dcon
      common/condat/dcon
      dcon=0.001
      ...
      subroutine matinv
c invert the matrix of the wwz equations...
      implicit none
      real*8 dmat
      real*8 dsol(0:2,0:2)
      real*8 dfac
...
c Write a similar header for this output file as is done for the formatted
c output of the BASIC code.
      implicit none
      character*80 filen
      real*8 tau,freq
      real*8 dt,dx
...
204   format(8h     tau,13h         freq,16h           WWZ  ,         12h         Amp,14h        m(ave),12h        Neff)
      ...
      real*8 dfre
c dnefff should probably be dneff, but we'll keep it for strict consistency...
      real*8 dnefff
      integer n1,n2
...
c go here once dz>0 (In BASIC version, this is the "OUTLOOP" section of SUB WWZ)
902     continue
        dpower=0.d0
        damp=0.d0
        do n1=0,ndim
...
c now, write everything out -- one write per frequency per tau...
...
206    format(f13.4,f11.4,f14.4,f11.4,f11.7,2(f11.4))
      ...
      enddo
c ...and close the output file when you're done.
      close(2)
      close(3)
      return
      end
