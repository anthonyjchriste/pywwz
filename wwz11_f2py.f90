...
subroutine maketau
    ...
    implicit none
    real(kind=8) tau, dt, dx
    real(kind=8) dave, dvar, dsig
    real(kind=8) dtspan, dtstep, dtaulo, dtauhi
    real(kind=8) taucheck
...
subroutine makefreq
    ...
    if(nfreq>1000) then
        ...
        read*, choice
        if(choice=='N'.or.choice=='n') goto 700
        if(choice/='Y'.and.choice/='y') goto 701
    endif
...
subroutine matinv
    ...
    do i = 0, ndim
        if(dmat(i, i)==0.d0) then
            if (i==ndim) return
            do j = i + 1, ndim
                if(dmat(j, i)/=0.d0) then
                    do k = 0, ndim
...
subroutine wwt
    implicit none
    real(kind=8) dt, dx, tau, freq, twopi, dcon
    real(kind=8) dvec(0:2), dmat, dcoef(0:2)
    real(kind=8) dave, dvar, dsig
    integer ndim
    integer itau1, itau2, ifreq1, ifreq2, nfreq, ntau
    integer i, j, itau, ifreq, idat, nstart, numdat
    real(kind=8) domega, dweight2, dz, dweight
    real(kind=8) dcc, dcw, dss, dsw, dxw, dvarw
    real(kind=8) dtau
    real(kind=8) dpower, dpowz, damp, dneff, davew
    real(kind=8) dfre
    ! dnefff should probably be dneff, but we'll keep it for strict consistency...
    real(kind=8) dnefff
    integer n1, n2

    real(kind=8) dmz, dmfre, dmper, dmpow, dmamp, dmcon, dmneff
    ...
    do itau = itau1, itau2
        ...
        do ifreq = ifreq1, ifreq2
            ...
            if(dneff>3.d0) then
                ...
                dpower = 0.d0
                damp = 0.d0
                if (dneff<1.d-9) dnefff = 0.d0
            endif
            if (damp<1.d-9) damp = 0.d0
            if (dpower<1.d-9) dpower = 0.d0
            if (dpowz<1.d-9) dpowz = 0.d0
